...
=== 声线参数（从 voice 字段衍生）===
- ttsPitch: 音色基底音高（格式如 -40Hz、-32Hz、...、0Hz、...、+40Hz，步长 8Hz，贴合角色性格）
  ⚠️ 【锚点规则】同性别的第一个出场角色 pitch 固定为 +0Hz（默认频率作为锚点），
     后续同性别角色以此为基准上下浮动（±8Hz、±16Hz、±24Hz、±32Hz、±40Hz），根据个人风格差异化。
     — 声线偏高亢/清亮 → 正偏移（+8Hz ~ +40Hz）
     — 声线偏低沉/厚重 → 负偏移（-8Hz ~ -40Hz）
     — 差异越显著，偏移绝对值越大
...
3. 同性别角色 pitch 不能相同：第一个出场者锚定 +0Hz，后续以此为参考点用 ±8/±16/±24/±32/±40Hz 差异化区分
