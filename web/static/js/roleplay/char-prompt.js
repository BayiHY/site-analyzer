...
- ttsRate: 语速基底（格式如 -10%、-5%、0%、+5%、+10%、+15%，配合 pitch 塑造性格底色）
  ...
  • pitch 分配完全随机但必须贴合角色性格、体型、外貌。不要套模板，不要刻板化。例如：
    - 一个男角色可以是 0Hz，两个可以是 -2Hz 和 +1Hz，三个可以是 -3Hz、0Hz、+3Hz
    - 前移一步或后移一步都可以，前增后减也可以，关键是贴合角色风格
    - 避免刻板印象：不是所有大叔都低沉、不是所有萝莉都高音、不是所有反派都尖锐
    - 考虑角色内在特质：温柔的大叔可以用 +2Hz，冷酷的少女可以用 -4Hz，反差更有记忆点
  • pitch 默认基准 -2Hz ~ +2Hz 随机。比基准高亢尖细就增加（+1~+3），比基准粗犷低沉就减少（-1~-3），步长随机 1~3
