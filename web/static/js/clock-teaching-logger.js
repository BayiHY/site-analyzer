// === Section: 钟表教具日志工具 ===
// 页内调试日志面板 — 与 audio-cutter 保持一致的底部展开条样式

window._clockLogEntries = window._clockLogEntries || [];
let _clockLogCollapsed = false;
let _clockLogPendingRender = false;

const CLOCK_LOG_TAGS = new Set([
    'INIT', 'TIME', 'DRAG', 'TOUCH', 'MODE', 'ERROR', 'WARN', 'DEBUG'
]);

const CLOCK_LEVEL_COLORS = { info: '#4fc3f7', warn: '#ffb74d', error: '#ef5350', debug: '#aaa' };

function clockLog(level, tag, msg) {
    const now = new Date();
    ...
    if (!CLOCK_LOG_TAGS.has(tag)) {
        console.log('[' + ts + '] [' + level.toUpperCase() + '] [' + tag + '] ' + msg);
        return;
    }

    console.log('[' + ts + '] [' + level.toUpperCase() + '] [' + tag + '] ' + msg);
    window._clockLogEntries.push(entry);
    scheduleClockLogRender();
}

window.clockLog = clockLog;

function scheduleClockLogRender() {
    if (_clockLogPendingRender) return;
    _clockLogPendingRender = true;
    requestAnimationFrame(() => {
        _clockLogPendingRender = false;
        renderClockLogPanel();
    });
}
...
function renderClockLogEntry(e) {
    const c = CLOCK_LEVEL_COLORS[e.level] || '#ccc';
...
function renderClockLogPanel() {
    ...
    if (bodyEntryCount < visibleEntries.length) {
        ...
        for (let i = bodyEntryCount; i < visibleEntries.length; i++) {
            ...
            const div = document.createElement('div');
            div.innerHTML = renderClockLogEntry(e);
            fragment.appendChild(div);
        }
        body.appendChild(fragment);
        body.scrollTop = body.scrollHeight;
    } else if (bodyEntryCount > visibleEntries.length) {
        body.innerHTML = visibleEntries.map(renderClockLogEntry).join('');
    }
}

// 初始化: 默认收起
_clockLogCollapsed = true;
clockLog('info', 'INIT', '日志工具已就绪');
