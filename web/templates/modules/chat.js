...
请保持角色一致性，不要跳出角色。`;
            ...
            const response = await agnesChat(messages);
            ...
            // 对话后场景图生成（技能规则：场景变化/装扮变化/情绪变化/特殊动作/记忆点 → 生图）
            try {
                const sceneDesc = parseSceneFromReply(response);
                if (sceneDesc && isSceneChanged(activeChar.name, sceneDesc)) {
                    addSceneGenStatus();
                    await generateSceneImage(activeChar.name, sceneDesc, activeChar);
                }
            } catch (e) {
            ...
            // 对话后让 LLM 评估情感变化
            try {
                await updateEmotions(activeChar.name, text, response);
            } catch (e) {
                console.warn('情感更新失败:', e);
            }

            // 对话后让 LLM 评估角色信息是否被玩家新发现
            try {
                await updateRevealedInfo(activeChar.name, text, response);
                // 刷新角色面板以显示新发现
            ...
            // 对话后让 LLM 动态更新角色的感知/秘密/当前情绪
            try {
                await updateDynamicAttributes(activeChar.name, text, response);
            } catch (e) {
