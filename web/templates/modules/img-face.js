...
// === Section: Agnes 图片生成 ===
    ...
    App.sanitizeImagePrompt = function(prompt, character) {
        ...
        // 统一追加艺术风格
        return appendArtStyle(cleaned.trim());
    }
    ...
    App.generateCharacterFace = async function(character, imagePrompt) {
        ...
        const safePrompt = sanitizeImagePrompt(imagePrompt, character);
        console.log('清洗后的生图 prompt:', safePrompt.slice(0, 150));

        let imageUrl;
        try {
            imageUrl = await agnesImageGen(safePrompt);
        } catch (e) {
            console.warn('生图失败，尝试使用备用 prompt:', e.message);
            // 失败后用基于角色信息的备用 prompt 重试
            const backupPrompt = buildBackupPrompt(character);
            console.log('备用 prompt:', backupPrompt.slice(0, 150));
            imageUrl = await agnesImageGen(backupPrompt);
        }
    ...
    App.generateSceneImage = async function(charName, sceneDesc, charObj) {
        ...
        try {
            ...
            const prompt = sceneToImagePrompt(sceneDesc, charObj, worldview);
            ...
            const faceUrl = getActiveCharacterFaceUrl();
            if (faceUrl) {
